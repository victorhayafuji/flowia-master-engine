...
>
> **Produto ativo:** MVP salão (`PRODUCT_LINE=salon`) · **Versão API:** 1.2.0 · **Última revisão doc:** Jul/2026 (doc v1.33)
>
...
- [**Parte VIII — Futuras implementações (NÃO MVP)**](docs/ROADMAP_FUTURO.md) — movida para `docs/ROADMAP_FUTURO.md`; **não implementar sem aprovação**
...
| **5 — Jornada e retenção pós-visita** | Receita perdida por falta de recall, histórico fragmentado, profissional sem contexto | — (futuro) | **Futuro** — ver [Parte VIII §42](docs/ROADMAP_FUTURO.md#42-epic-customer-journey-intelligence) |
...
Funcionalidades futuras (jornada inteligente, régua pós-atendimento, etc.): matriz dedicada na [Parte VIII §43](docs/ROADMAP_FUTURO.md#43-índice-consolidado-de-itens-futuros) — **não fazem parte do MVP**.
...
| Anamnese / NPS pós-atendimento | **DEFERIDO** | Schema (`anamnesis_*`, `recall_days`) existe; fluxo não implementado — ver [Parte VIII §42](docs/ROADMAP_FUTURO.md#42-epic-customer-journey-intelligence) |
| **Futuras implementações (todas)** | **Futuro** | Índice consolidado na [Parte VIII §43](docs/ROADMAP_FUTURO.md#43-índice-consolidado-de-itens-futuros) — **não implementar** sem aprovação explícita |
| Reagendamento inteligente (no-show / atraso) | **Parcial** | **F3 implementado** (cliente reagenda/cancela o próprio agendamento via agente — `reschedule_time`/`cancel_appointment`). Recuperação proativa, cascata de atraso e reativação seguem **futuro** — [Parte VIII §49](docs/ROADMAP_FUTURO.md#49-epic-reagendamento-inteligente--recuperação-de-no-showatraso-documentação--não-implementar) |
...
> ⚠️ **Deprecação OpenAI:** a família `gpt-4o` está em fim de vida (retirada do ChatGPT em fev/2026; variantes API como `chatgpt-4o-latest` já aposentadas). `gpt-4o-mini`, `gpt-4o` e `text-embedding-3-small` seguem disponíveis na API direta, mas em trajetória de deprecação. Migração planejada para a linha `gpt-5.x mini` — ver [Parte VIII §48.1](docs/ROADMAP_FUTURO.md#481-migração-de-modelos-openai-4o--5x). **Não trocar `MODEL_NAME`/`VISION_MODEL_NAME` sem executar o plano de migração.**
...
| Rate limiting geral (slowapi) + cooldowns | In-process (memória do worker) | **Pré-requisito de escala:** mover para backend compartilhado (Postgres/Redis) antes de `scale>1` — ver [Parte VIII §48.3](docs/ROADMAP_FUTURO.md#483-rate-limiting-distribuído-pré-requisito-de-scale1) |
...
- **Scrollbar custom (Jul/2026):** thumb fino translúcido (`color-mix` de `--foreground` 18%) com `--accent` no hover, track transparente — props padrão `scrollbar-width`/`scrollbar-color` + fallback `::-webkit-scrollbar` no `index.css`. Sem gradiente no thumb (`scrollbar-color` só aceita cor sólida).
- **Convenção de superfícies (3 camadas — Jul/2026):** `glass-panel` = vidro **decorativo** que coexiste com o fundo (cards no fluxo); `card-brutal`/`--surface` = superfície **sólida** (inputs, tabelas); `glass-overlay` (`--surface-overlay`, ~97% opaco + blur cosmético) = overlay que **bloqueia e substitui a atenção** (drawer mobile, modal, sheet). **Nunca** usar `glass-panel` como corpo de overlay fixo de navegação — 4,5% de opacidade vaza o conteúdo de trás. Escala z-index nomeada em comentário no `index.css` (10 fluxo / 40 backdrop / 50 overlay / 60 toast).
...
| 6 — Customer Journey Intelligence | **Futuro** | [Parte VIII §42](docs/ROADMAP_FUTURO.md#42-epic-customer-journey-intelligence) · [`docs/ROADMAP.md`](docs/ROADMAP.md) Cap. 6 |
| 7 — Reagendamento Inteligente (no-show / atraso) | **Futuro** | [Parte VIII §49](docs/ROADMAP_FUTURO.md#49-epic-reagendamento-inteligente--recuperação-de-no-showatraso-documentação--não-implementar) · [`docs/ROADMAP.md`](docs/ROADMAP.md) Cap. 7 |
...
| [`docs/ROADMAP_FUTURO.md`](docs/ROADMAP_FUTURO.md) | **Parte VIII** — futuras implementações detalhadas (§41–§52); **não implementar sem aprovação** |
...
| 1.33 | Jul/2026 | **Scrollbar custom GAUSSIX**: scrollbar nativa clara ("Windows 95") substituída por thumb fino translúcido com accent roxo no hover — props padrão `scrollbar-width`/`scrollbar-color` (Chrome 121+/Firefox/Safari 18.2+) + fallback `::-webkit-scrollbar` (pílula flutuante via `background-clip: padding-box`); cores por `color-mix()` sobre tokens (`--foreground` 18% / `--accent` 55%), sem hex duplicado; CSS-only em `index.css` (§31) |
| 1.32 | Jul/2026 | **Mobile polish (Onda 2)**: microinterações do drawer (`spring-transition`, fade do backdrop com `onTransitionEnd`, traço lateral `--accent-2` no item ativo), a11y `inert` no conteúdo de trás via hook `useInert` (React 18.3 não aceita `inert` como prop JSX — set direto no node), copy de status "Em atendimento"→"Atendendo" unificada (Overview + `appointmentStatus.ts` da Agenda), KPI 3 colunas com mini-header (prefixos fora das células), empty states com ícone, flash `--success` ao trocar status, tabs financeiro `border` em mobile; +1 assert `inert` no `mobile-drawer.spec.ts` (18/18). `useIsMobile` avaliado e dispensado (sem condicional JS que o justificasse) |
| 1.31 | Jul/2026 | **Mobile visual hardening (Onda 1)**: nova camada de superfície `glass-overlay` (`--surface-overlay` ~97% opaco) para drawer/modal/sheet + convenção das 3 superfícies e escala z-index documentadas (§31, comentário no `index.css`); drawer mobile opaco com backdrop `bg-black/70`+blur, header some quando aberto (fim do wordmark duplicado), touch targets ≥44px, safe-area; Overview mobile: painéis `max-h-[60vh]`, nomes `line-clamp-2`, bordas 1px em itens densos, chip de data discreto; novo spec `e2e/mobile-drawer.spec.ts` (viewport 412px, 5 testes). Onda 2 (polish) documentada no plano — não implementada |
...
> **Conteúdo movido para [`docs/ROADMAP_FUTURO.md`](docs/ROADMAP_FUTURO.md)** — para manter esta fonte da verdade enxuta e focada no MVP ativo (Partes I–VII).
>
> **GUARDRAIL — AGENTES E DEVS:** todo o conteúdo de lá é **somente visão estratégica**. **Não implementar** nada (código, migrations, endpoints, tabelas, prompts de produção, UI) **sem pedido explícito** do usuário com aprovação de produto. As seções **§41–§52** (Customer Journey Intelligence, Reagendamento Inteligente, Modernização de stack e blueprints) preservam a numeração e os anchors originais lá.
