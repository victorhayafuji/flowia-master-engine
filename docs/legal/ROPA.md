...
| 14 | Lacunas de conhecimento (`knowledge_gaps`) | Pergunta do cliente (texto), tipo de agente | Cliente | Observabilidade RAG (perguntas sem resposta) | Legítimo interesse | **365 dias** desde `last_seen_at` (`KNOWLEDGE_GAPS_RETENTION_DAYS`; purge diário APScheduler) | RLS tenant, captura fail-soft |
